...
async def show_new_game_bet_choices(message: Message, game_type: str, stake: int):
    ...
    descriptions = {
        ...
        "frog": "Перепрыгивайте по кувшинкам и доберитесь до другого берега.",
    }
    await message.answer(
        f"╭────────────────────╮\n       {title}\n╰────────────────────╯\n\n"
        f"💎 Ставка: <b>{money(stake)} ₽</b>\n\n"
...
@dp.callback_query(F.data.regexp(r"^(darts|football|basketball|plinko|knb|russian_roulette|21|frog)_stake_\d+$"))
...
@dp.callback_query(F.data.in_(set(NEW_GAME_COEFFICIENTS) | {"plinko_roll", "21_start", "frog_start"}))
async def new_game_play(callback: CallbackQuery):
    ...
    # Russian roulette: 6 chambers, selected number are bullets.
    if game_type == "russian_roulette":
        ...
        games.pop(user_id, None)
        await safe_callback_answer(callback, "💥 Выстрел!" if fired else "😮 Пусто!")
        await callback.message.answer(
    ...
    if game_type == "21" and action == "21_start":
        deck = ["A"] * 4 + ["J"] * 4 + ["Q"] * 4 + ["K"] * 4 + [str(i) for i in range(6, 11)] * 4
        random.shuffle(deck)
        ...
        builder.adjust(1)
        await callback.message.answer(
            f"🃏 <b>21</b>\n\nТвои карты: <b>{' '.join(player)}</b>\nСумма: <b>{_hand_value(player)}</b>\n\nКоэффициент выигрыша: <b>x1.8</b>",
            reply_markup=builder.as_markup(),
        )
        return

    # Frog: five jumps, 50/50 each; x2 for reaching the shore.
    if game_type == "frog" and action == "frog_start":
        safe = all(random.random() >= 0.5 for _ in range(5))
        multiplier = 2.0 if safe else 0
        payout = int(round(stake * multiplier)) if safe else 0
        if payout: change_balance(user_id, payout)
        record_game(user_id, "frog", stake, "win" if payout else "loss", multiplier, payout)
        games.pop(user_id, None)
        await safe_callback_answer(callback)
        await callback.message.answer(
            "🐸 <b>ЖАБА</b>\n\n" +
            (f"🎁 Жаба добралась до другого берега!\n💰 Выигрыш: <b>{money(payout)} ₽</b> (x2)" if payout else "🌊 Жаба упала в воду. <b>Проигрыш.</b>") +
            f"\n\n💳 Баланс: <b>{money(get_balance(user_id))} ₽</b>",
            reply_markup=after_game_keyboard("frog"),
        )
        return
...
async def game_21_action(callback: CallbackQuery):
    ...
    if callback.data == "21_hit":
        ...
        await safe_callback_answer(callback)
        await callback.message.answer(
            f"🃏 <b>21</b>\n\nТвои карты: {' '.join(game['player'])} = <b>{value}</b>\n"
            f"Карты бота: {' '.join(game['dealer'])} = <b>{dealer_value}</b>\n\n"
    ...
    await callback.message.edit_text(
        f"🃏 <b>21</b>\n\nТвои карты: <b>{' '.join(game['player'])}</b>\nСумма: <b>{_hand_value(game['player'])}</b>\n\nКоэффициент выигрыша: <b>x1.8</b>",
        reply_markup=builder.as_markup(),
    )
