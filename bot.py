...
async def api_me(payload: WebAppAuth):
    ...
    return {"user": user, "balance": get_balance(uid), "guard": guard, "stats": stats}
...
async def api_game_action(payload: WebGameActionRequest):
    ...
    if game["type"] == "frog":
        ...
        if pos != game["frog_safe"][level]:
            return _web_finish(uid, "frog", game["stake"], False, 0, {"level": level, "selected": pos})
        game["level"] += 1
        if game["level"] >= 4:
            return _web_finish(uid, "frog", game["stake"], True, 23.43, {"level": 4, "selected": pos})
        mults = [1.14, 1.87, 4.68, 23.43]
