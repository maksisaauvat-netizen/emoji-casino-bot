...
CUSTOM_EMOJI = {
    "logo": "5235989279024373566",
    "vip": "5471952986970267163",
    "balance": "6025976946083500432",
    "action": "5470177992950946662",
    "win": "5188344996356448758",
    "loss": "5454350746407419714",
    "dice": "5260547274957672345",
    "slots": "5235989279024373566",
    "bowling": "5370853837689070338",
    "roulette": "5226711870492126219",
    "mines": "5280569974404966639",
    "crash": "5188481279963715781",
    "stake": "5471952986970267163",
    "winnings": "5255981634527704754",
    "profile": "5116116063188157350",
    "history": "5472279086657199080",
    "deposit": "5267300544094948794",
}
...
def premiumize_text(text: str) -> str:
    ...
    replacements = {
        "💰 Выигрыш": tg_emoji(CUSTOM_EMOJI["winnings"], "💰") + " Выигрыш",
        "💰 ВЫИГРЫШ": tg_emoji(CUSTOM_EMOJI["winnings"], "💰") + " ВЫИГРЫШ",
        "💰 +": tg_emoji(CUSTOM_EMOJI["winnings"], "💰") + " +",
        "🏆": tg_emoji(CUSTOM_EMOJI["win"], "🏆"),
        "💥": tg_emoji(CUSTOM_EMOJI["loss"], "💥"),
        ...
        "💎": tg_emoji(CUSTOM_EMOJI["stake"], "💎"),
        "💰": tg_emoji(CUSTOM_EMOJI["balance"], "💰"),
        "👤": tg_emoji(CUSTOM_EMOJI["profile"], "👤"),
        "📜": tg_emoji(CUSTOM_EMOJI["history"], "📜"),
        "💳": tg_emoji(CUSTOM_EMOJI["deposit"], "💳"),
    }

    # Protect replacements so the fallback emoji inside a generated tag
...
async def roulette_stake(
    ...
    # Гарантируем наличие пользователя в БД перед игрой.
    ensure_user(user_id)
...
async def confirm_roulette(
    ...
    # Пользователь должен существовать до списания ставки/записи результата.
    ensure_user(user_id)
    ...
    for frame in animation:
        try:
            await message.edit_text(premiumize_text(frame))
        except Exception:
            pass
