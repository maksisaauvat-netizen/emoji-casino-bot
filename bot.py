...
async def profile_handler(callback: CallbackQuery):
    ...
    builder.button(text="💳  ПЛАТЕЖИ", callback_data="my_payments")
    builder.button(text="💸  МОИ ВЫВОДЫ", callback_data="my_withdrawals")
    builder.button(text="НАЗАД", callback_data="back_main")
    builder.adjust(1)

    profile_text = (
        "╭────────────────────╮\n"
        f"       {tg_emoji('5116116063188157350', '👤')} | PROFILE\n"
        "╰────────────────────╯\n\n"
        f"{tg_emoji('5471952986970267163', '💰')} | BALANCE:\n"
        f"└ ‘{money(balance)}’\n\n"
        f"{tg_emoji('5426896538062332283', '🎮')} | ИГРЫ\n"
        f"└ ‘{games_played}’\n\n"
        f"{tg_emoji('5188344996356448758', '🏆')} | ПОБЕДЫ\n"
        f"└ ‘{wins}’\n\n"
        f"{tg_emoji('5454350746407419714', '❌')} | ПОРАЖЕНИЯ\n"
        f"└ ‘{losses}’\n\n"
        f"{tg_emoji('6033106828018062225', '📈')} | WINRATE\n"
        f"└ ‘{winrate}%’\n\n"
        f"{tg_emoji('5429651785352501917', '💵')} | СТАВКИ\n"
        f"└ ‘{money(total_bet)} ₽’\n\n"
        f"{tg_emoji('5278467510604160626', '🏆')} | ВЫИГРАНО\n"
        f"└ ‘{money(total_won)} ₽’\n\n"
        f"{tg_emoji('5375452661036358740', '🔥')} | MAX WIN\n"
        f"└ ‘{money(biggest_win)} ₽’"
    )
