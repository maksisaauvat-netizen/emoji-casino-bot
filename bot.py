...
# Serve Mini App static assets (case images, icons, etc.).
# The HTML references these files as /assets/... .
app.mount(
    "/assets",
...
# Connected Mini App clients. WebSocket is used only as a synchronization
# channel; all authoritative state still comes from the database.
websocket_clients = set()

games = {}
# Отдельное состояние игр Mini App, чтобы Telegram callback-игры не конфликтовали с web-сессиями.
web_games = {}
...
ROULETTE_MULTIPLIERS = {
    ...
    "dozen2": 2.7,
    "dozen3": 2.7,
    **{f"number_{n}": 35.0 for n in range(37)},
}
...
def roulette_bet_wins(selection: str, number: int) -> bool:
    if selection.startswith("number_"):
        try:
            return int(selection.split("_", 1)[1]) == number
...
class AdminActionRequest(WebAppAuth):
    action: str = Field(min_length=1, max_length=64)
    user_id: Optional[int] = Field(default=None, gt=0)
    amount_rub: Optional[int] = Field(default=None, gt=0, le=100000000)
    withdrawal_id: Optional[int] = Field(default=None, gt=0)
...
class CaseOpenRequest(WebAppAuth):
    case_key: str = Field(min_length=1, max_length=64)

class InventorySellRequest(WebAppAuth):
...
class WebGameStartRequest(WebAppAuth):
    game: str = Field(min_length=1, max_length=64)
    stake_rub: int = Field(gt=0, le=1000000)
    option: Optional[str] = Field(default=None, max_length=64)
    mines_count: Optional[int] = Field(default=None, ge=2, le=24)
    bombs: Optional[int] = Field(default=None, ge=1, le=4)

class WebGameActionRequest(WebAppAuth):
    action: str = Field(min_length=1, max_length=64)
    index: Optional[int] = Field(default=None, ge=0, le=24)
...
def _web_sync_payload(uid: int) -> dict:
    """Build the authoritative state sent to a Mini App sync client."""
    ensure_user(uid)
    ...
    payload = {
        ...
        "history": history,
        "inventory": get_inventory(uid, 100),
        "cases": get_case_history(uid, 30),
    }
    # A compact revision lets the client know that this is a fresh server snapshot.
    payload["revision"] = hashlib.sha256(
        json.dumps({
            "balance": payload["balance"],
            "stats": stats,
            "games": history["games"][:3],
            "payments": history["payments"][:3],
            "withdrawals": history["withdrawals"][:3],
            "inventory": get_inventory(uid, 5),
            "cases": get_case_history(uid, 5),
...
async def api_sync(websocket: WebSocket):
    """Near-real-time Mini App synchronization over an authenticated WebSocket.

    The connection is authenticated with Telegram initData in the query string.
    The server polls the authoritative DB snapshot internally and pushes only
    when the revision changes, so the browser does not have to poll /api/me.
    """
    ...
    try:
        ...
        await websocket.close(code=1008, reason="Invalid Telegram init data")
        return
    ...
    try:
        ...
        print("WEBAPP SYNC ERROR:", repr(error))
        try:
...
async def api_me(payload: WebAppAuth):
    ...
    guard = get_webapp_guard(uid)
    stats = get_user_stats(uid)
    return {"user": user, "balance": get_balance(uid), "guard": guard, "stats": stats, "is_admin": is_admin(uid)}
...
@app.post("/api/admin")
async def api_admin(payload: WebAppAuth):
    user = validate_telegram_webapp_data(payload.init_data)
    uid = int(user["id"])
    if not is_admin(uid):
        raise HTTPException(status_code=403, detail="Admin access required")
    users = _rows("""
        SELECT user_id, balance_rub, created_at, updated_at
        FROM users ORDER BY updated_at DESC LIMIT 100
    """)
    for item in users:
        item["stats"] = get_user_stats(int(item["user_id"]))
    withdrawals = get_pending_withdrawals(50)
    summary = {
        "users": len(_rows("SELECT user_id FROM users")),
        "balance": int(sum(int(x["balance_rub"]) for x in users)),
        "pending_withdrawals": len(withdrawals),
        "pending_amount": int(sum(int(x["amount_rub"]) for x in withdrawals)),
    }
    return {"summary": summary, "users": users, "withdrawals": withdrawals}


@app.post("/api/admin/action")
async def api_admin_action(payload: AdminActionRequest):
    user = validate_telegram_webapp_data(payload.init_data)
    ...
    if action in {"add_balance", "remove_balance"}:
        if payload.user_id is None or payload.amount_rub is None:
            raise HTTPException(status_code=400, detail="user_id and amount_rub are required")
        amount = int(payload.amount_rub)
        if action == "remove_balance":
            ok = subtract_balance(int(payload.user_id), amount)
            if not ok:
                raise HTTPException(status_code=400, detail="Insufficient user balance")
        else:
            change_balance(int(payload.user_id), amount)
        return {"ok": True, "user_id": int(payload.user_id), "balance": get_balance(int(payload.user_id))}
    if action in {"approve_withdrawal", "reject_withdrawal", "pay_withdrawal"}:
        if payload.withdrawal_id is None:
            raise HTTPException(status_code=400, detail="withdrawal_id is required")
        wid = int(payload.withdrawal_id)
        ...
        if result is None:
            raise HTTPException(status_code=400, detail="Withdrawal state does not allow this action")
        return {"ok": True, "withdrawal": result}
    if action == "user":
        if payload.user_id is None:
            raise HTTPException(status_code=400, detail="user_id is required")
        target = int(payload.user_id)
        ensure_user(target)
        return {"ok": True, "user_id": target, "balance": get_balance(target), "stats": get_user_stats(target), "games": get_game_history(target, 20), "payments": get_payment_history(target, 20), "withdrawals": get_withdrawal_history(target, 20)}
...
CASE_CATALOG = {
    "starter":{"name":"Starter Case","price":1000,"tag":"START","items":[{"key":"starter_chip","name":"Starter Chip","rarity":"common","value_rub":350,"weight":55,"icon":"◇"},{"key":"blue_echo","name":"Blue Echo","rarity":"uncommon","value_rub":800,"weight":28,"icon":"◆"},{"key":"frost_shard","name":"Frost Shard","rarity":"rare","value_rub":1800,"weight":12,"icon":"✦"},{"key":"aurora_core","name":"Aurora Core","rarity":"epic","value_rub":4500,"weight":4,"icon":"✧"},{"key":"resonant_crown","name":"Resonant Crown","rarity":"legendary","value_rub":12000,"weight":1,"icon":"♛"}]},
    "blue":{"name":"Blue Case","price":2500,"tag":"HOT","items":[{"key":"blue_chip","name":"Blue Chip","rarity":"common","value_rub":900,"weight":52,"icon":"◇"},{"key":"ice_prism","name":"Ice Prism","rarity":"uncommon","value_rub":1900,"weight":30,"icon":"◆"},{"key":"deep_wave","name":"Deep Wave","rarity":"rare","value_rub":4200,"weight":13,"icon":"✦"},{"key":"polar_core","name":"Polar Core","rarity":"epic","value_rub":9500,"weight":4,"icon":"✧"},{"key":"blue_crown","name":"Blue Crown","rarity":"legendary","value_rub":26000,"weight":1,"icon":"♛"}]},
    "silver":{"name":"Silver Case","price":5000,"tag":"ELITE","items":[{"key":"silver_mark","name":"Silver Mark","rarity":"common","value_rub":1800,"weight":50,"icon":"◇"},{"key":"moon_glass","name":"Moon Glass","rarity":"uncommon","value_rub":3900,"weight":31,"icon":"◆"},{"key":"silver_flame","name":"Silver Flame","rarity":"rare","value_rub":8500,"weight":14,"icon":"✦"},{"key":"starlight","name":"Starlight","rarity":"epic","value_rub":18000,"weight":4,"icon":"✧"},{"key":"silver_crown","name":"Silver Crown","rarity":"legendary","value_rub":50000,"weight":1,"icon":"♛"}]},
    "ice":{"name":"Ice Case","price":10000,"tag":"FROST","items":[{"key":"ice_coin","name":"Ice Coin","rarity":"common","value_rub":3500,"weight":50,"icon":"◇"},{"key":"glacier","name":"Glacier","rarity":"uncommon","value_rub":7800,"weight":31,"icon":"◆"},{"key":"frozen_sun","name":"Frozen Sun","rarity":"rare","value_rub":17000,"weight":14,"icon":"✦"},{"key":"ice_heart","name":"Ice Heart","rarity":"epic","value_rub":38000,"weight":4,"icon":"✧"},{"key":"ice_crown","name":"Ice Crown","rarity":"legendary","value_rub":100000,"weight":1,"icon":"♛"}]},
    "crystal":{"name":"Crystal Case","price":25000,"tag":"HOT","items":[{"key":"crystal_dust","name":"Crystal Dust","rarity":"common","value_rub":8500,"weight":49,"icon":"◇"},{"key":"amethyst","name":"Amethyst","rarity":"uncommon","value_rub":19000,"weight":32,"icon":"◆"},{"key":"prism","name":"Prism","rarity":"rare","value_rub":42000,"weight":14,"icon":"✦"},{"key":"void_crystal","name":"Void Crystal","rarity":"epic","value_rub":95000,"weight":4,"icon":"✧"},{"key":"crystal_crown","name":"Crystal Crown","rarity":"legendary","value_rub":250000,"weight":1,"icon":"♛"}]},
    "royal":{"name":"Royal Case","price":50000,"tag":"PREMIUM","items":[{"key":"royal_coin","name":"Royal Coin","rarity":"common","value_rub":17000,"weight":48,"icon":"◇"},{"key":"royal_gem","name":"Royal Gem","rarity":"uncommon","value_rub":38000,"weight":33,"icon":"◆"},{"key":"royal_flame","name":"Royal Flame","rarity":"rare","value_rub":85000,"weight":14,"icon":"✦"},{"key":"royal_relic","name":"Royal Relic","rarity":"epic","value_rub":190000,"weight":4,"icon":"✧"},{"key":"resonant_royal","name":"Resonant Royal","rarity":"legendary","value_rub":500000,"weight":1,"icon":"♛"}]},
}

def _case_roll(case_data):
    total=sum(int(x["weight"]) for x in case_data["items"]); pick=secrets.randbelow(total); cursor=0
    for item in case_data["items"]:
        cursor+=int(item["weight"])
        if pick<cursor: return item
    return case_data["items"][-1]


@app.get("/api/cases")
async def api_cases():
    return {"cases":[{"key":key,"name":data["name"],"price":data["price"],"tag":data["tag"],"items":[{k:v for k,v in item.items() if k!="weight"}|{"chance":item["weight"]} for item in data["items"]]} for key,data in CASE_CATALOG.items()]}

@app.post("/api/case/open")
async def api_case_open(payload: CaseOpenRequest):
    user=validate_telegram_webapp_data(payload.init_data); uid=int(user["id"])
    if payload.case_key not in CASE_CATALOG: raise HTTPException(status_code=404, detail="Case not found")
    _web_require_game_access(uid); data=CASE_CATALOG[payload.case_key]; item=_case_roll(data)
    result=create_case_open(uid,payload.case_key,data["name"],data["price"],item)
    if result is None: raise HTTPException(status_code=400, detail="Недостаточно средств")
    return {"status":"finished","case":{"key":payload.case_key,"name":data["name"],"price":data["price"]},"item":{k:result[k] for k in ("id","key","name","rarity","value_rub","icon","created_at")},"balance":result["balance"],"inventory":get_inventory(uid,100)}

@app.post("/api/inventory")
async def api_inventory(payload: WebAppAuth):
    user=validate_telegram_webapp_data(payload.init_data); uid=int(user["id"])
    return {"inventory":get_inventory(uid,100),"cases":get_case_history(uid,30),"balance":get_balance(uid)}

@app.post("/api/inventory/sell")
async def api_inventory_sell(payload: InventorySellRequest):
    user=validate_telegram_webapp_data(payload.init_data); uid=int(user["id"])
    result=sell_inventory_item(uid,payload.item_id)
    if result is None: raise HTTPException(status_code=404, detail="Предмет уже продан или не найден")
    return {"ok":True,**result,"inventory":get_inventory(uid,100)}


WEB_GAME_NAMES = {
    "slots", "darts", "football", "bowling", "basketball", "plinko", "dice",
    "mines", "tower", "knb", "russian_roulette", "roulette", "21", "frog",
}

WEB_DICE_OPTIONS = {
    "dice_single_less": ("Меньше 4", 1.85),
    "dice_single_more": ("Больше 3", 1.85),
    "dice_double_less": ("Меньше 7", 2.05),
    "dice_double_more": ("Больше 7", 2.05),
    "dice_double_seven": ("Ровно 7", 5.0),
}

WEB_SIMPLE_OPTIONS = {
    "darts": {"red": 2.3, "white": 2.3, "center": 4.7, "miss": 4.7},
    "football": {"goal": 1.5, "miss": 2.3, "post": 4.7},
    "basketball": {"goal": 2.3, "miss": 2.3, "stuck": 4.7},
    "bowling": {"hit": 2.0, "miss": 2.0},
}


def _web_require_game_access(uid: int):
    if REAL_ECONOMY:
        require_real_money_access(uid, "game")


def _web_balance(uid: int):
    return int(get_balance(uid))


def _web_finish(uid: int, game_type: str, stake: int, won: bool, multiplier: float = 0.0, extra=None, payout_override: Optional[int] = None):
    payout = int(round(stake * multiplier)) if won and multiplier > 0 else 0
    if payout_override is not None:
        payout = max(0, int(payout_override))
    if payout:
        change_balance(uid, payout)
    record_game(uid, game_type, stake, "win" if won else "loss", multiplier if won else 0, payout)
    web_games.pop(uid, None)
    result = {
        "status": "finished",
        "game": game_type,
        "stake": stake,
        "won": bool(won),
        "multiplier": float(multiplier if won else 0),
        "payout": payout,
        "balance": _web_balance(uid),
    }
    if extra:
        result.update(extra)
    return result
...
async def api_game_start(payload: WebGameStartRequest):
    user = validate_telegram_webapp_data(payload.init_data)
    uid = int(user["id"])
    ensure_user(uid)
    _web_require_game_access(uid)
    game = payload.game.strip().lower()
    stake = int(payload.stake_rub)
    if game not in WEB_GAME_NAMES:
        raise HTTPException(status_code=400, detail="Unknown game")
    if uid in web_games:
        raise HTTPException(status_code=409, detail="Сначала завершите текущую игру")
    if not subtract_balance(uid, stake):
        raise HTTPException(status_code=400, detail="Недостаточно средств")

    # One-shot games. RNG is generated only on the server.
    if game in {"darts", "football", "basketball", "bowling"}:
        ...
        if game == "darts":
            ...
            outcome = "hit" if value >= 4 else "miss"
        option = (payload.option or "").replace(f"{game}_", "")
        if option not in WEB_SIMPLE_OPTIONS[game]:
            change_balance(uid, stake)
            raise HTTPException(status_code=400, detail="Выберите вариант ставки")
        won = outcome == option
        multiplier = WEB_SIMPLE_OPTIONS[game][option]
        return _web_finish(uid, game, stake, won, multiplier if won else 0, {"value": value, "outcome": outcome, "option": option})

    if game == "dice":
        option = payload.option or ""
        if option not in WEB_DICE_OPTIONS:
            change_balance(uid, stake)
            raise HTTPException(status_code=400, detail="Выберите прогноз DICE")
        if option.startswith("dice_single"):
            v1 = random.randint(1, 6)
            total = v1
            won = total < 4 if option == "dice_single_less" else total > 3
            values = [v1]
        else:
            v1, v2 = random.randint(1, 6), random.randint(1, 6)
            total = v1 + v2
            won = total < 7 if option == "dice_double_less" else total > 7 if option == "dice_double_more" else total == 7
            values = [v1, v2]
        multiplier = WEB_DICE_OPTIONS[option][1]
        return _web_finish(uid, "dice", stake, won, multiplier if won else 0, {"values": values, "total": total, "option": option})
    ...
    if game == "slots":
        ...
        if value == 64:
            ...
            multiplier = 0
        return _web_finish(uid, "slots", stake, multiplier > 0, multiplier, {"value": value, "symbols": symbols})

    if game == "plinko":
        result = random.randint(1, 6)
        multiplier = {1: 0, 2: .3, 3: .9, 4: 1.1, 5: 1.4, 6: 1.95}[result]
        # x0.3/x0.9 возвращают часть ставки, но это всё ещё проигрыш
        # с точки зрения результата/статистики. x1.0+ — выигрыш.
        won = multiplier >= 1.0
        payout = int(round(stake * multiplier))
        return _web_finish(uid, "plinko", stake, won, multiplier, {"result": result}, payout_override=payout)

    if game == "knb":
        option = (payload.option or "").replace("knb_", "")
        if option not in {"rock", "scissors", "paper"}:
            change_balance(uid, stake)
            raise HTTPException(status_code=400, detail="Выберите ход КНБ")
        bot_choice = random.choice(["rock", "scissors", "paper"])
        result = _knb_result(option, bot_choice)
        won = result == "win"
        return _web_finish(uid, "knb", stake, won, 2.8 if won else 0, {"player": option, "bot": bot_choice, "result": result})

    if game == "russian_roulette":
        option = (payload.option or "").replace("rr_", "")
        try: bullets = int(option)
        except ValueError: bullets = 0
        if bullets not in range(1, 6):
            change_balance(uid, stake)
            raise HTTPException(status_code=400, detail="Выберите количество пуль")
        multiplier = {1: 1.14, 2: 1.4, 3: 1.9, 4: 2.8, 5: 5.7}[bullets]
        fired = random.randrange(6) < bullets
        return _web_finish(uid, "russian_roulette", stake, not fired, multiplier if not fired else 0, {"bullets": bullets, "fired": fired})

    if game == "roulette":
        option = payload.option or ""
        if option not in ROULETTE_MULTIPLIERS:
            change_balance(uid, stake)
            raise HTTPException(status_code=400, detail="Выберите ставку рулетки")
        result = random.choice(ROULETTE_NUMBERS)
        won = roulette_bet_wins(option, result)
        multiplier = ROULETTE_MULTIPLIERS[option]
        return _web_finish(uid, "roulette", stake, won, multiplier if won else 0, {"result": result, "color": roulette_result_color(result), "selection": option})

    if game == "mines":
        mine_count = int(payload.mines_count or 2)
        if not MINES_MIN <= mine_count <= MINES_MAX:
            change_balance(uid, stake)
            raise HTTPException(status_code=400, detail="Количество мин: 2–24")
        mines = random.sample(range(MINES_SIZE), mine_count)
        web_games[uid] = {"type": "mines", "stake": stake, "mines": mines, "opened": [], "multiplier": 1.0, "settled": False}
        return {"status": "playing", "game": "mines", "stake": stake, "mines_count": mine_count, "opened": [], "multiplier": 1.0, "balance": _web_balance(uid)}

    if game == "tower":
        bombs = int(payload.bombs or 1)
        if bombs not in TOWER_COEFFICIENTS:
            change_balance(uid, stake)
            raise HTTPException(status_code=400, detail="Количество бомб: 1–4")
        web_games[uid] = {"type": "tower", "stake": stake, "bombs": bombs, "level": 0, "tower_bombs": [random.sample(range(5), bombs) for _ in range(10)], "settled": False}
        return {"status": "playing", "game": "tower", "stake": stake, "bombs": bombs, "level": 0, "multiplier": 1.0, "balance": _web_balance(uid)}
    ...
    if game == "21":
        ...
        player = [deck.pop(), deck.pop()]
        dealer = [deck.pop(), deck.pop()]
        web_games[uid] = {"type": "21", "stake": stake, "deck": deck, "player": player, "dealer": dealer, "settled": False}
        return {"status": "playing", "game": "21", "stake": stake, "player": player, "dealer": [dealer[0], "?"] , "player_value": _hand_value(player), "balance": _web_balance(uid)}

    if game == "frog":
        web_games[uid] = {"type": "frog", "stake": stake, "level": 0, "frog_safe": [random.randrange(5) for _ in range(4)], "settled": False}
        return {"status": "playing", "game": "frog", "stake": stake, "level": 0, "multiplier": 1.0, "balance": _web_balance(uid)}

    change_balance(uid, stake)
    raise HTTPException(status_code=400, detail="Game is not implemented")


@app.post("/api/game/cancel")
async def api_game_cancel(payload: WebAppAuth):
    user = validate_telegram_webapp_data(payload.init_data)
    uid = int(user["id"])
    ensure_user(uid)
    _web_require_game_access(uid)
    game = web_games.pop(uid, None)
    if not game:
        return {"cancelled": False, "balance": _web_balance(uid)}
    change_balance(uid, int(game["stake"]))
    return {"cancelled": True, "game": game["type"], "refund": int(game["stake"]), "balance": _web_balance(uid)}


@app.post("/api/game/action")
async def api_game_action(payload: WebGameActionRequest):
    user = validate_telegram_webapp_data(payload.init_data)
    uid = int(user["id"])
    ensure_user(uid)
    _web_require_game_access(uid)
    game = web_games.get(uid)
    if not game:
        raise HTTPException(status_code=409, detail="Нет активной игры")
    action = payload.action
    ...
    if game["type"] == "mines":
        if action == "cashout":
            ...
            multiplier = float(game["multiplier"])
            return _web_finish(uid, "mines", game["stake"], True, multiplier, {"opened": game["opened"], "mines": game["mines"], "mines_count": len(game["mines"])})
        if not action.startswith("open:"):
            raise HTTPException(status_code=400, detail="Неизвестное действие")
        index = int(action.split(":", 1)[1])
        if not 0 <= index < MINES_SIZE or index in game["opened"]:
            raise HTTPException(status_code=400, detail="Некорректная клетка")
        if index in game["mines"]:
            result = _web_finish(uid, "mines", game["stake"], False, 0, {"hit_mine": index, "mines": game["mines"], "opened": game["opened"], "mines_count": len(game["mines"])})
            return result
        game["opened"].append(index)
        game["multiplier"] = mines_multiplier(len(game["mines"]), len(game["opened"]))
        if len(game["opened"]) >= MINES_SIZE - len(game["mines"]):
            return _web_finish(uid, "mines", game["stake"], True, game["multiplier"], {"opened": game["opened"], "mines": game["mines"], "mines_count": len(game["mines"])})
        return {"status": "playing", "game": "mines", "opened": game["opened"], "multiplier": game["multiplier"], "payout": int(round(game["stake"] * game["multiplier"])), "balance": _web_balance(uid)}

    if game["type"] == "tower":
        if action == "cashout":
            level = int(game["level"])
            if level <= 0: raise HTTPException(status_code=400, detail="Пройдите хотя бы один уровень")
            multiplier = TOWER_COEFFICIENTS[game["bombs"]][level - 1]
            return _web_finish(uid, "tower", game["stake"], True, multiplier, {"level": level, "bombs": game["bombs"], "tower_bombs": game["tower_bombs"]})
        if not action.startswith("pick:"): raise HTTPException(status_code=400, detail="Неизвестное действие")
        index = int(action.split(":", 1)[1])
        level = int(game["level"])
        if level >= 10 or index not in range(5): raise HTTPException(status_code=400, detail="Некорректный выбор")
        if index in game["tower_bombs"][level]:
            return _web_finish(uid, "tower", game["stake"], False, 0, {"level": level + 1, "bomb": index, "bombs": game["bombs"], "tower_bombs": game["tower_bombs"]})
        game["level"] += 1
        if game["level"] >= 10:
            multiplier = TOWER_COEFFICIENTS[game["bombs"]][-1]
            return _web_finish(uid, "tower", game["stake"], True, multiplier, {"level": 10, "bombs": game["bombs"], "tower_bombs": game["tower_bombs"]})
        multiplier = TOWER_COEFFICIENTS[game["bombs"]][game["level"] - 1]
        return {"status": "playing", "game": "tower", "level": game["level"], "bombs": game["bombs"], "multiplier": multiplier, "balance": _web_balance(uid)}
    ...
    if game["type"] == "21":
        if action == "hit":
            ...
            value = _hand_value(game["player"])
            if value > 21:
                return _web_finish(uid, "21", game["stake"], False, 0, {"player": game["player"], "dealer": game["dealer"], "player_value": value, "dealer_value": _hand_value(game["dealer"])})
            return {"status": "playing", "game": "21", "player": game["player"], "dealer": [game["dealer"][0], "?"], "player_value": value, "balance": _web_balance(uid)}
        if action == "stand":
            while _hand_value(game["dealer"]) < 17:
                game["dealer"].append(game["deck"].pop())
            pv = _hand_value(game["player"]); dv = _hand_value(game["dealer"])
            won = pv <= 21 and (dv > 21 or pv > dv)
            return _web_finish(uid, "21", game["stake"], won, 1.8 if won else 0, {"player": game["player"], "dealer": game["dealer"], "player_value": pv, "dealer_value": dv})
        raise HTTPException(status_code=400, detail="Неизвестное действие")

    if game["type"] == "frog":
        if not action.startswith("jump:"): raise HTTPException(status_code=400, detail="Неизвестное действие")
        pos = int(action.split(":", 1)[1]); level = int(game["level"])
        if pos not in range(5) or level >= 4: raise HTTPException(status_code=400, detail="Некорректный прыжок")
        if pos != game["frog_safe"][level]:
            return _web_finish(uid, "frog", game["stake"], False, 0, {"level": level, "selected": pos, "frog_safe": game["frog_safe"]})
        game["level"] += 1
        if game["level"] >= 4:
            return _web_finish(uid, "frog", game["stake"], True, 23.43, {"level": 4, "selected": pos, "frog_safe": game["frog_safe"]})
        mults = [1.14, 1.87, 4.68, 23.43]
        return {"status": "playing", "game": "frog", "level": game["level"], "multiplier": mults[game["level"]-1], "balance": _web_balance(uid)}

    raise HTTPException(status_code=400, detail="Игра не поддерживает действия")


@app.post("/api/game/round")
async def api_game_round(payload: GameRoundRequest):
    # Backward-compatible endpoint: стартует игру без дополнительных опций.
    return await api_game_start(WebGameStartRequest(init_data=payload.init_data, game=payload.game, stake_rub=payload.stake_rub))
