...
def main_keyboard(user_id: int):
    """Главное меню: только Профиль / Игры / Кошелек."""
...
async def manual_stake_start(
    ...
    if game_type == "mines":
        state["mines_count"] = int(last_mines_count.get(user_id, 2))
...
async def handle_stake_message(
    ...
    if game_type == "mines":
        games[user_id]["mines_count"] = int(state.get("mines_count", last_mines_count.get(user_id, 2)))
    # DICE
    if game_type == "dice":
...
async def mines_start(callback: CallbackQuery):
    user_id = callback.from_user.id
    games[user_id] = {"type": "mines_setup"}
...
async def mines_stake(callback: CallbackQuery):
    ...
    game = games.get(user_id, {})
    mine_count = int(game.get("mines_count", 2))
...
async def confirm_mines(callback: CallbackQuery):
    ...
    stake = int(game["stake"])
    mine_count = int(game["mines_count"])
