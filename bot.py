...
async def start_handler(message: Message):
    ...
    await message.answer(
        "🎰 <b>RESONANT — CASINO</b>\n\n"
        "Добро пожаловать!\n\n"
        "💰 Играй в мини-игры за внутренние рубли ₽.\n\n"
        "Выбери действие:",
...
async def create_deposit(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        "Оплати счёт и нажми "
        "«ПРОВЕРИТЬ ОПЛАТУ».\n\n"
        "Система также проверяет оплату автоматически.",
...
async def dice_stake(callback: CallbackQuery):
    ...
    kb.button(
        text="🎲 ДВА КУБИКА",
        callback_data=f"dice_two_stake_{stake}"
    )
...
async def dice_play(callback: CallbackQuery):
    ...
    payout = int(stake * 1.85) if win else 0
...
@dp.callback_query(
    F.data.startswith("dice_two_stake_")
)
...
async def dice_two_play(callback: CallbackQuery):
    ...
    payout = int(stake * multiplier) if multiplier else 0
...
async def roulette_game(callback: CallbackQuery):
    ...
    for stake in STAKES:
        kb.button(
            text=f"🎯 {stake} ₽",
            callback_data=f"roulette_stake_{stake}"
        )
...
async def roulette_result(callback: CallbackQuery):
    user_id = callback.from_user.id
...
async def bowling_game(callback: CallbackQuery):
    ...
    for stake in STAKES:
        kb.button(
            text=f"🎳 {stake} ₽",
            callback_data=f"bowling_stake_{stake}"
        )
...
async def bowling_result(callback: CallbackQuery):
    ...
    payout = int(stake * multiplier) if win else 0
...
async def crash_game(callback: CallbackQuery):
    ...
    for stake in STAKES:
        kb.button(
            text=f"📈 {stake} ₽",
            callback_data=f"crash_stake_{stake}"
        )
...
async def catch_all(message: Message):
    ...
    if (
        state
        and state.get("type") == "admin_search"
