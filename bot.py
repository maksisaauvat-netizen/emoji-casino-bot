...
API_SESSION_MAX_AGE = int(os.getenv("API_SESSION_MAX_AGE", "86400"))
...
def _telegram_init_data_user(init_data: str):
    ...
    try:
        ...
        if not auth_date or abs(int(time.time()) - auth_date) > API_SESSION_MAX_AGE:
            return None
        ...
        print("TELEGRAM INIT DATA ERROR:", repr(error))
        return None


def _api_user(request: Request):
    """Resolve authenticated Telegram user. Demo auth is opt-in for local/staging."""
    ...
    if not REAL_ECONOMY and os.getenv("ALLOW_DEMO_API", "false").lower() == "true":
        user_id = int(request.headers.get("X-Demo-User", DEMO_USER_ID))
        ensure_user(user_id)
        return {"id": user_id, "first_name": "Demo", "username": "demo"}
...
def _api_auth_error():
    return JSONResponse(
        {"ok": False, "error": "telegram_auth_required"},
        status_code=401,
    )
...
async def api_config():
    return {
        ...
        "min_withdrawal_rub": MIN_WITHDRAWAL,
    }
...
async def api_me(request: Request):
    ...
    return {
        "ok": True,
        "user": user,
        "balance_rub": get_balance(user_id),
        "real_economy": REAL_ECONOMY,
    }
...
async def api_history(request: Request):
    ...
    return {
        "ok": True,
        "games": get_game_history(user_id, 20),
        "payments": get_payment_history(user_id, 20),
        "withdrawals": get_withdrawal_history(user_id, 20),
    }
...
async def api_deposit(request: Request):
    ...
    if not user:
        return _api_auth_error()
    if not REAL_ECONOMY:
        return JSONResponse({"ok": False, "error": "real_economy_disabled"}, status_code=403)
    ...
    try:
        invoice = await create_invoice(int(user["id"]), amount_usdt)
    except Exception as error:
        print("API CREATE INVOICE ERROR:", repr(error))
        return JSONResponse({"ok": False, "error": "invoice_creation_failed"}, status_code=502)

    return {"ok": True, "invoice": invoice}


@app.post("/api/deposit/{invoice_id}/check")
async def api_deposit_check(invoice_id: int, request: Request):
    user = _api_user(request)
    if not user:
        return _api_auth_error()
    if not REAL_ECONOMY:
    ...
    try:
        ...
        print("API CHECK INVOICE ERROR:", repr(error))
        return JSONResponse({"ok": False, "error": "payment_check_failed"}, status_code=502)

    invoice = get_invoice(invoice_id)
    if invoice and int(invoice.get("user_id", user["id"])) != int(user["id"]):
        return JSONResponse({"ok": False, "error": "forbidden"}, status_code=403)

    return {"ok": True, "result": result, "invoice": invoice}
...
async def api_withdraw(request: Request):
    ...
    try:
        ...
        payout_details = str(body.get("payout_details", "")).strip()
        if amount_rub < MIN_WITHDRAWAL or len(payout_details) < 5:
            raise ValueError
    except Exception:
        return JSONResponse({"ok": False, "error": "invalid_withdrawal"}, status_code=400)

    user_id = int(user["id"])
    if get_balance(user_id) < amount_rub:
        return JSONResponse({"ok": False, "error": "insufficient_balance"}, status_code=400)
    ...
    try:
        ...
        print("API CREATE WITHDRAWAL ERROR:", repr(error))
        withdrawal = None

    if withdrawal is None:
        return JSONResponse({"ok": False, "error": "withdrawal_creation_failed"}, status_code=400)

    return {"ok": True, "withdrawal": withdrawal, "balance_rub": get_balance(user_id)}


@app.post("/api/game/round")
async def api_game_round(request: Request):
    """Server-authoritative simple 2x game.

    RTP is the configured mathematical target: win probability 0.4825 * 2.0 = 0.965.
    Real-money execution remains disabled unless REAL_ECONOMY=true.
    """
    user = _api_user(request)
    if not user:
        return _api_auth_error()
    ...
    try:
        body = await request.json()
        stake = int(body.get("stake_rub", 0))
    ...
    user_id = int(user["id"])
    # In demo mode the ledger is deliberately untouched.
    if not REAL_ECONOMY:
        win = secrets.randbelow(100000) < 48250
        return {
            "ok": True,
            "demo": True,
            "game": "resonant-classic",
            "stake_rub": stake,
            "result": "WIN" if win else "LOSS",
            "multiplier": 2.0 if win else 0.0,
            "payout_rub": stake * 2 if win else 0,
            "rtp_target": RTP_TARGET,
        }
    ...
    # 48.25% * 2.0 = 96.5% expected return.
    win = secrets.randbelow(100000) < 48250
...
async def web_index():
    ...
    return {"status": "ok", "bot": "Resonant Casino"}
