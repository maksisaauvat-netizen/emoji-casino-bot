...
async def new_game_play(callback: CallbackQuery):
    ...
    # Жаба — анимация прыжков по кувшинкам
    if game_type == "frog" and action == "frog_start":
        safe = all(random.random() >= 0.5 for _ in range(5))
        await safe_callback_answer(callback)
        animation = await callback.message.answer("🐸 <b>ЖАБА</b>\n\n🌊  🪷  🪷  🪷  🪷  🪷  🏝️\n\n🐸 Готовится к прыжку...")
        if safe:
            ...
            for pos in positions[1:]:
                ...
                if pos <= 5:
                    cells[pos - 1] = "🐸"
                await animation.edit_text("🐸 <b>ЖАБА</b>\n\n🌊  " + "  ".join(cells) + "  🏝️\n\n💨 Прыжок!")
            multiplier = 2.0
            payout = int(round(stake * multiplier))
        else:
            fail_at = random.randint(1, 5)
            for pos in range(1, fail_at + 1):
                await asyncio.sleep(0.55)
                cells = ["🪷"] * 5
                if pos < fail_at:
                    cells[pos - 1] = "🐸"
