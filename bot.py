...
def local_banner(filename: str):
    # Prefer the dedicated banners/ directory, then the legacy root path.
    candidates = [BASE_DIR / "banners" / filename, BASE_DIR / filename]
...
PHOTO_MAIN = local_banner("main_menu.png") or local_banner("main.png") or os.getenv("PHOTO_MAIN", "")
PHOTO_GAMES = local_banner("games.png") or PHOTO_MAIN or os.getenv("PHOTO_GAMES", "")
...
PHOTO_DARTS = local_banner("darts.png")
PHOTO_FOOTBALL = local_banner("football.png")
PHOTO_BOWLING = local_banner("bowling.png")
PHOTO_BASKETBALL = local_banner("basketball.png")
PHOTO_PLINKO = local_banner("plinko.png")
PHOTO_SPECIAL_GAMES = local_banner("special_games.png")
PHOTO_TOWER = local_banner("tower.png")
PHOTO_KNB = local_banner("knb.png")
PHOTO_RUSSIAN_ROULETTE = local_banner("russian_roulette.png")
...
PHOTO_21 = local_banner("21.png")
PHOTO_FROG = local_banner("frog.png")
...
async def show_new_game_bet_choices(message: Message, game_type: str, stake: int):
    ...
    photo = NEW_GAME_PHOTOS.get(game_type)
    if photo:
        await message.answer_photo(
            photo=photo,
            caption=premiumize_text(caption),
            reply_markup=new_game_choice_keyboard(game_type),
            parse_mode=ParseMode.HTML,
        )
    else:
        await message.answer(
            premiumize_text(caption),
            reply_markup=new_game_choice_keyboard(game_type),
        )
...
NEW_GAME_PHOTOS = {
    "darts": PHOTO_DARTS,
    "football": PHOTO_FOOTBALL,
    "basketball": PHOTO_BASKETBALL,
    "plinko": PHOTO_PLINKO,
    "knb": PHOTO_KNB,
    "russian_roulette": PHOTO_RUSSIAN_ROULETTE,
    "21": PHOTO_21,
    "frog": PHOTO_FROG,
    "tower": PHOTO_TOWER,
}
...
@dp.callback_query(F.data.in_({"game_darts", "game_football", "game_basketball", "game_plinko", "game_knb", "game_russian_roulette", "game_21", "game_frog", "game_tower"}))
async def new_game_start(callback: CallbackQuery):
    ...
    await edit_or_answer(
        ...
        NEW_GAME_PHOTOS.get(game_type, PHOTO_GAMES),
    )
...
# TOWER is handled by new_game_start above.
...
async def noop(callback: CallbackQuery):
    ...
    try:
        await edit_or_answer(
            callback,
            "⭐ <b>СПЕЦИАЛЬНЫЕ ИГРЫ</b>\n\nВыберите игру:",
            games_keyboard(),
            PHOTO_SPECIAL_GAMES,
        )
    except Exception as error:
        print("SPECIAL GAMES BANNER ERROR:", repr(error))
